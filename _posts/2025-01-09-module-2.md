...
The goal is not just to create a chatbot that replies — but one that **follows a conversational flow**, knows when to stop asking questions, and returns a **parsed final result**.

## Expected Behavior

The chatbot you build should simulate a natural, helpful conversation where the assistant gradually collects all the necessary travel information. The assistant should guide the user through a sequence of questions and remember previous responses, so it doesn't repeat itself.

Here is the expected behavior step-by-step:
...
- Throughout the interaction, it should collect the following key details from the user:
...
- When the user confirms (e.g. with `yes`, `generate`, or `finalize`), the assistant must **produce a structured output** with the following fields, in JSON format.

This flow ensures that the chatbot does not generate a response prematurely and only switches to the output phase when the user is ready.

## Final Output Format

Once the user has provided all the necessary travel details and confirms they’re ready, your chatbot must generate a final structured response in JSON format. This output should summarize the user’s trip in a way that can be easily processed by other systems, such as a frontend or a reporting tool.

You’ll use a Pydantic model to define the expected structure. This ensures your response includes all the required fields and follows a consistent format:
...
1. Conversational Memory with `MemorySaver`
...
2. Message Trimming with `trim_messages`
...
3. Streaming Responses with `.stream()`
...
4. Structured Output with Output Parser

At the end of the interaction, your chatbot must generate a well-formatted JSON travel plan — not just raw text.

You must guide the LLM with a clear prompt, and optionally use a  `pydantic.BaseModel` to validate and format the final result, matching the required schema.
