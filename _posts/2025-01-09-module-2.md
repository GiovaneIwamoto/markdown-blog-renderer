...
`.stream()` → processes an input and streams the output incrementally, allowing you to work with partial results as they are produced.

`.astream_log()` → streams both the final output and selected intermediate steps for deeper tracing and debugging.
