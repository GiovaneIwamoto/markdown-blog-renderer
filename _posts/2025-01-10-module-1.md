...
### **Working with LLM APIs: Beyond OpenAI**  
In this course, we’ll use the **OpenAI API** as a starting point to learn how to interact with large language models (LLMs) programmatically. 

But I want to make something clear from the beginning:

Our goal is not to focus on OpenAI-specific functions or methods like `chat.completions`. Instead, I want you to understand the general structure and principles behind interacting with LLM APIs — because this knowledge applies across different AI providers, not just OpenAI.

#### **Why OpenAI’s API? A De Facto Standard**  
OpenAI’s API design — with endpoints like `/v1/chat/completions`, JSON request/response formats, and parameters like `temperature` and `max_tokens` — has become an industry benchmark.

Many AI providers now offer **OpenAI-compatible APIs**.

#### **What "OpenAI-Compatible" Really Means**

It means that these providers:

- **Use the same endpoint structure** (e.g., same HTTP routes)
- **Accept the same request payload** (e.g., JSON with model, messages, etc.)
- **Return the same response format**

When providers like **Mistral, Together AI, or DeepSeek** say they’re "OpenAI-compatible," it means: 

1. **They mimic OpenAI’s API design**, *not* that they use OpenAI’s models.  

  - Example: Mistral’s `mistral-large` is their own model, but its API *behaves* like OpenAI’s.
  
2. **You can reuse tools** built for OpenAI (e.g., LangChain’s ChatOpenAI, OpenAI’s SDK) by just updating:  
  ```python
  ...
  client = OpenAI(
    ...
    base_url="https://api.mistral.ai/v1"  # Not OpenAI’s URL!
  )
  ```

3. **Zero dependency on OpenAI**: The models, infrastructure, and costs are entirely separate.  

#### **The Power of Abstraction: LangChain**

To future-proof your skills, we’ll use **LangChain**—an abstraction layer that:  

- **Unifies APIs**: Interact with OpenAI, Mistral, Anthropic, etc., through one interface.
- **Avoids vendor lock-in**: Switch providers by changing a config, not rewriting logic.
- **Focuses on workflows**: Learn *how to integrate LLMs*—not just one API’s quirks.
...
llm = ChatOpenAI(
  api_key="any_key",
  base_url="https://api.together.xyz/v1",  # Redirect to another provider
  model="togethercomputer/llama-2-70b-chat"  # Not an OpenAI model!
)
```

#### **Key Takeaway**  

OpenAI’s API is a **standard**, not just a product. By learning its patterns, you’re learning how to interact with *any* modern LLM API. This approach ensures you’re adaptable as the ecosystem evolves.  

- **Write once, run anywhere**: Your code works across OpenAI, Mistral, Together AI, local models, and more—just by switching configuration values.

- **Leverage the ecosystem**: Seamlessly use existing tools like LangChain, LlamaIndex, OpenAI SDKs, without needing rewrites or new learning curves.

This mindset shift—from "OpenAI as a product" to "OpenAI as a standard"—will make you a more versatile developer, capable of integrating AI models across different providers without being tied to one specific platform.

*Note: This is a playlist containing three videos.*

{% include embed.html url="https://www.youtube.com/embed/hcyOajJfSLs?si=zRzO05pHqcmiYDcd" %}
