...
This leads us to a critical application of embeddings in **Retrieval-Augmented Generation** — a powerful technique that enhances language models by grounding their responses in external information. In a RAG pipeline, when a user submits a query, it is first embedded into a vector. This query vector is then compared against a pre-indexed set of document embeddings stored in a vector database. Instead of relying on exact keyword matches, the system retrieves documents whose semantic meaning closely aligns with the user’s input — even if the words used are different.
...
Step 1 – Convert Text to Vectors (Indexing): First, all the documents (or chunks of documents) in your knowledge base are converted into embedding vectors using an embedding model. These vectors are stored in a vector database, and each vector is associated with the original text it came from (so we can retrieve the text later). This is usually done ahead of time as a preparation step.

Step 2 – Convert Query to Vector (Retrieval): When a user asks a question (the query), that question is also converted into a vector using the same embedding model. Now we have a query vector representing the meaning of the question.
