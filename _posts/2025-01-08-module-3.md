...
> *Note: Everything we've covered so far—generating embeddings from text, storing those embeddings in a vector database, and preparing them for future search—falls under the process known as **indexing**. This step involves taking your raw text data, converting it into vector embeddings, and organizing them so they can be efficiently retrieved later through similarity search.*
